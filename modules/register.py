...
def recruiter_check(root):
    ...
    if name1 and email1 and pwd1 and cpwd1:
        ...
        if email1 in exist_email:
            messagebox.showinfo('ALERT!', 'Email is already registered, please login!')
            email.delete(0, END)
        ...
        messagebox.showinfo('ALERT!', 'Please provide all the values!')
...
def client_check(root):
    ...
    if name1 and email1 and pwd1 and cpwd1:
        ...
        if email1 in exist_email:
            messagebox.showinfo('ALERT!', 'Email already registered, please login!')
            email.delete(0, END)
...
def client_complete(root):
    ...
    workxp = Entry(r3, placeholder='Work Experience(in yrs)', width=20)
...
def client_submit(root):
    ...
    if gender1 and age1 and loc1 and workxp1:
        ...
        try:
            mycon = sql.connect(host='localhost', port='3306', user='root',
                                password=user_pwd, database='mydb')
            cur = mycon.cursor()
            cur.execute(exe)
            cur.execute(exe1)
            name.delete(0, END)
